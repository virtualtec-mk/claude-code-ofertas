...
- **Prosa de bloguero, no compresión telegráfica.** Esta regla cuelga del manifiesto 2.octies y aplica especialmente al **párrafo del precio** y a las **zonas de matiz** (descuento, historial, comparativa). En ABC el registro es señorial pero cercano: las frases largas con punto y coma encadenando tres ideas se reescriben en frases más cortas con marcadores de oralidad que respiran ("Ojo, eso sí", "También conviene tener en cuenta que", "Si no hay prisa", "lo cierto es que", "sinceramente"). Veta elipsis técnica suelta como cierre: "es la parada lógica", "abre la puerta a", "marca un antes y un después", "se apoya en", "marca la diferencia", "no es un dato menor". Si las usas, el dato concreto va inmediatamente al lado.
- **AliExpress: contexto narrativo, no apartado.** Si el producto procede de AliExpress y el lector puede salir engañado por el precio "anterior" del listado, ese contexto se integra en el cierre del artículo con buena narrativa: una o dos frases que aclaren la referencia real (PVP oficial europeo si existe, o precio absoluto y comparación con competidores en Amazon si no existe). Nunca como sección H2 propia ni como listado de reglas dentro del cuerpo. Lo que **sí** está vetado en cualquier parte del artículo: usar como gancho el % de descuento del listado de AliExpress (titular, subtítulo, intro, H2 o cuerpo). En recopilatorios cuya tienda hilo es AliExpress, el hilo conductor se reconstruye sin apoyarse en porcentajes ("ofertas por debajo de X euros en AliExpress", "productos de menos de 30 euros que aguantan", no "descuentos del 60% en AliExpress"). Si el angle-picker propuso `liquidacion` apoyado en ese %, el writer recalibra al ángulo más cercano que no requiera el porcentaje (`uso-practico`, `recomendacion-personal`).
