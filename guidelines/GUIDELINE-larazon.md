...
- **REGLA UNIVERSAL (manifiesto 2.bis.bis):** la intro **narra, suena a la persona-redactora declarada en el frontmatter, e invita a leer la segunda frase**. La primera frase del cuerpo nunca comprime "[producto] cae/baja/se rebaja a X euros / un X% en [tienda]" — ese formato está vetado en cuota 0 en **todos los ángulos**, incluidos `liquidacion` y `precio-psicologico`. La cifra de precio, si abre, lo hace como muy pronto en la segunda frase del primer párrafo, con escena delante.
- **El arranque depende del ángulo y de la persona-redactora.** No hay una fórmula única.
  - En `liquidacion` / `precio-psicologico`: el primer párrafo puede tener el precio como protagonista, pero **siempre con escena, observación o anécdota delante**. Patrones tipo *"A estas alturas de la temporada…"*, *"Llega ese momento del año en el que…"*, *"Cualquiera que lleve un par de años haciendo X conoce la escena…"*.
...
- **Prosa de bloguero, no compresión telegráfica.** Esta regla cuelga del manifiesto 2.octies y aplica especialmente al **párrafo del precio** y a las **zonas de matiz** (descuento, historial, comparativa con generación anterior). En La Razón el registro es periodístico cercano: las frases largas con punto y coma encadenando tres ideas (✗ "está en Amazon por 380€; el descuento se apoya en el precio recomendado, así que sobre lo habitual la bajada es modesta") **se reescriben en dos párrafos con marcadores de oralidad** (✓ "ya se puede encontrar en Amazon por algo menos de 380 euros. Ojo, eso sí: el descuento que aparece toma como referencia el precio recomendado…"). Usa "Ojo, eso sí", "También hay que tener en cuenta que", "Si no tienes prisa", "quizá tenga sentido", "sinceramente", "la verdad es que" cuando aporten respiración — al menos uno o dos por artículo, sin abusar. Veta elipsis técnica suelta como cierre: "es la parada lógica", "abre la puerta a", "marca un antes y un después", "se apoya en", "marca la diferencia". Si las usas, el dato concreto va inmediatamente al lado.
- **AliExpress: contexto narrativo, no apartado.** Si el producto procede de AliExpress y el lector puede salir engañado por el precio "anterior" del listado, ese contexto se integra en el cierre del artículo con buena narrativa: una o dos frases que aclaren la referencia real (PVP oficial europeo si existe, o precio absoluto y comparación con competidores en Amazon si no existe). Nunca como sección H2 propia ni como listado de reglas dentro del cuerpo. Lo que **sí** está vetado en cualquier parte del artículo: usar como gancho el % de descuento del listado de AliExpress (titular, subtítulo, intro, H2 o cuerpo). Si el angle-picker propuso `liquidacion` apoyado en ese %, el writer recalibra al ángulo más cercano que no requiera el porcentaje (`uso-practico`, `recomendacion-personal`).
...
12. **Placeholders y negritas markdown en el draft (manifiesto 2.septies).** Vetado en cuota 0:
    - `[Widget pricebox]`, `[widget]`, `[pricebox]`, `[CTA]`, `[iframe]` o cualquier marcador equivalente del widget de compra. El pricebox themonetise.es lo inserta el redactor en el CMS en las dos posiciones definidas (tras el primer párrafo del cuerpo y antes del disclaimer).
    - `![Imagen principal](...)`, `![Imagen del producto](...)`, cualquier sintaxis markdown de imagen. La imagen principal la inserta el CMS por anclaje 2, no por marcador.
    - `*Foto: Marca*`, `*Foto: Amazon*`, `*Imagen: …*`, `*Pie: …*` y cualquier pie de foto suelto. El pie lo añade el CMS junto a la imagen.
    - `**texto en negrita**` y `__texto en negrita__` para resaltar palabras clave o frases. La Razón ya rechazaba negritas markdown desde v2.3; queda escrito aquí en cuota 0 explícita. El énfasis se consigue con orden de la frase y vocabulario, no con asteriscos.

    Si cualquiera de estos elementos aparece en el draft, el editor-in-chief lo elimina sin reemplazo y sin avisar (el log de correcciones solo lo registra agregado: "eliminados N placeholders/negritas residuales"). El writer no necesita señalizar dónde van: el redactor lo sabe por la guideline y por la plantilla del CMS.
