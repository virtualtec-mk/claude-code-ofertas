---
name: crear-articulo
description: Orquesta el flujo completo de redacción de un artículo de oferta desde una o varias URLs de producto, o desde una ficha del inbox/ generada por /buscar-ofertas. Coordina los subagentes product-researcher, angle-picker, headline-generator, writer y editor-in-chief para producir un draft listo para revisión, tanto en modo mono-producto como en guías de compra multi-producto.
argument-hint: <url1> [url2 ... urlN] [medio] | [medio] [filtro-inbox]
...
`$ARGUMENTS` admite dos modos:

### Modo A — URLs explícitas (prioritario)

- Una o varias URLs de producto y, opcionalmente, el slug del medio.
...
### Modo B — Inbox (fallback si no hay URLs)

Si en `$ARGUMENTS` NO hay tokens que empiecen por `http://` o `https://`, se entra en modo inbox:

- Sin argumentos o solo `MEDIO`: lista completa del inbox para que el redactor elija con número.
- Con `MEDIO` y un segundo token: se trata el segundo token como **filtro de búsqueda en `inbox/`** (fuzzy match en nombre de archivo, case-insensitive).
  - Ejemplos:
    - `/crear-articulo larazon` → muestra menú con todos los `inbox/*.md`.
    - `/crear-articulo larazon xiaomi` → filtra por "xiaomi". Si hay 1 match único, usa esa ficha directamente. Si hay varios, muestra menú filtrado.
    - `/crear-articulo abc auriculares-anc` → filtra por "auriculares-anc".
...
```
URLS = [tokens que empiezan por http:// o https://]
N_URLS = len(URLS)
NO_URL_TOKENS = [tokens que NO son URLs]
```

- Si `N_URLS >= 1` → **Modo A**:
  - `MEDIO` = el token de `NO_URL_TOKENS` que coincide con un slug de `guidelines/GUIDELINE-*.md` (si lo hay).
  - Resto: ignorar (avisar si hay tokens raros).
- Si `N_URLS == 0` → **Modo B (inbox)**:
  - `MEDIO` = primer token de `NO_URL_TOKENS` que coincide con slug de medio.
  - `FILTRO_INBOX` = el siguiente token de `NO_URL_TOKENS` (puede estar vacío).
  - Ejecuta el sub-paso **PASO 1.5 — Resolución de inbox** antes del PASO 1.

> Para el resto del flujo, cuando solo hay una URL (`N_URLS=1`), usa `URL_PRODUCTO = URLS[0]`. Cuando hay varias, opera sobre la lista `URLS` completa. Cuando vienes de Modo B, `URLS` se rellena con la `url_producto` extraída del frontmatter de la ficha del inbox elegida, y el flujo continúa como Modo A desde el PASO 2.

---

## PASO 1.5 — Resolución de inbox (solo Modo B)

Solo se ejecuta cuando `N_URLS == 0`.

1. Lista `inbox/*.md` (excluye `.gitkeep`).
2. Si no hay ficheros: muestra al redactor *"El inbox está vacío. Pásame una URL o lanza primero `/buscar-ofertas` para generar fichas."* y detén el flujo.
3. Si hay `FILTRO_INBOX`:
   - Calcula `candidatos = [f for f in inbox/*.md if FILTRO_INBOX.lower() in f.lower()]`.
   - Si `len(candidatos) == 1`: usa esa ficha sin pausa. Avisa al redactor: *"Encontrado en inbox: {nombre}. Continúo."*.
   - Si `len(candidatos) > 1`: muestra menú filtrado y deja elegir.
   - Si `len(candidatos) == 0`: avisa *"Sin coincidencias para '{FILTRO_INBOX}' en inbox/."* y muestra el menú completo como fallback.
4. Si no hay `FILTRO_INBOX`: muestra el menú completo.

Formato del menú:

```
Fichas disponibles en inbox/:
  1. 27-05-2026-xiaomi-smart-air-fryer-4-5l.md   — Xiaomi Smart Air Fryer 4.5L (aliexpress · larazon)
  2. 26-05-2026-philips-oneblade-pro.md          — Philips OneBlade Pro (amazon · mundodeportivo)
  ...
¿Cuál?
```

El "lado derecho" (título, anunciante, medio sugerido) se extrae del frontmatter de cada ficha: `titulo`, `anunciante`, `medio`.

5. Cuando el redactor elija una ficha:
   - Lee su frontmatter completo.
   - Extrae `url_producto` → asígnalo a `URLS[0]`.
   - Si el frontmatter trae `medio` y `MEDIO` está vacío, úsalo.
   - Si `MEDIO` viene del input y discrepa del frontmatter, prevalece el del input (avisa: *"La ficha sugiere medio '{medio_ficha}'; uso el que pasaste: '{MEDIO}'."*).
   - Guarda en variable de estado `INBOX_FICHA_PATH` la ruta del fichero usado (para registrarla en el log de la sesión).
6. Continúa el flujo normal desde el PASO 1 con `URLS` y `MEDIO` rellenos. NO vuelvas a investigar la URL con `product-researcher` por duplicado: el writer usa `product-researcher` igualmente (esa es la fuente de verdad para redacción), pero ahora el redactor puede contrastar el resultado con la ficha del inbox si quiere (queda en `INBOX_FICHA_PATH`).

---

## PASO 0 — Sincronizar con el repositorio (fallback)

> **Nota:** desde el merge 27/05/2026, el repo tiene un hook `SessionStart` (`.claude/hooks/session-start-pull.sh`) que hace `git pull --ff-only` automáticamente al abrir Claude Code. Este PASO 0 queda como **fallback defensivo** dentro del skill por si la sesión se está reusando o el hook no se ejecutó.
